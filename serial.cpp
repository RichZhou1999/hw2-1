...
void apply_force(particle_t& particle, particle_t& neighbor, bool same) {
    // Calculate Distance
    ...
    particle.ay += coef * dy;
    if (!same) {
        // Don't want to double count if doing same box
        neighbor.ax -= coef * dx;
        neighbor.ay -= coef * dy;
    }
...
void apply_force_bin(int row, int column, int row2, int column2, int lda, bool same = false){
...
void apply_force_bins(int row, int column, int lda){
    ...
    apply_force_bin(row, column, row, column, lda, true);
    // right neighbor
